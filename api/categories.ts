 ...
 */
export const getServerCategory = async (id: RecordId): Promise<Category | null> => {
  const { $sdb } = useNuxtApp();
  try {
    const category = await $sdb.select<Category>(id);
    return category;
  } catch (e) {
    console.error(e);
    return null;
...
export const getCategory = async (id: RecordId): Promise<Category | null> => {
  try {
...
export const getServerCategories = async (): Promise<Category[]> => {
  const { $sdb } = useNuxtApp();
  try {
    const categories = await $sdb.select<Category>("Categories");
    return categories;
  } catch (e) {
    console.error(e);
    return [];
